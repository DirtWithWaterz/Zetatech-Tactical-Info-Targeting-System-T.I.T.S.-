...
public class Splash : MonoBehaviour
{
    ...
    IEnumerator splish()
    {
        ...
        {
            // Calculate the percentage of time passed (value from 0 to 1)
            float t = timeElapsed / etaDuration;

            // (Optional) Add easing so it starts/stops smoothly
            // t = Mathf.SmoothStep(0f, 1f, t);

            // Interpolate position
            eta.localPosition = Vector3.Lerp(etaStart, etaFinish, t);

            // Increment time by the duration of the last frame
        ...
        // Ensure the object reaches the exact final destination
